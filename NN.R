...
model_5<-neuralnet(strength~cement+slag+ash+water+superplastic+coarseagg+fineagg+age,data= concrete_norm,linear.output = F,hidden = 2)
plot(model_5)
model_5_res<-compute(model_5,concrete_norm)
pred_strn_5<-model_5_res$net.result
cor(pred_strn_5,concrete_norm$strength)
